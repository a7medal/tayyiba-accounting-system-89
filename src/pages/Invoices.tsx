...
import { PlusCircle, FileSearch, FileText, Download, Trash2, Edit } from 'lucide-react';
              ...
              <TableBody>
                {selectedInvoice?.items.map((item: any, index: number) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-center">{(item.quantity * item.price).toLocaleString()} ريال</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          ...
          <div className="flex justify-end">
            <div className="w-72 space-y-2">
              <div className="flex justify-between">
                ...
                <span>{selectedInvoice?.amount.toLocaleString()} ريال</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">ضريبة القيمة المضافة (15%):</span>
                <span>{(selectedInvoice?.amount * 0.15).toLocaleString()} ريال</span>
              </div>
              <div className="flex justify-between pt-2 border-t">
                <span className="font-bold">الإجمالي:</span>
                <span className="font-bold">{(selectedInvoice?.amount * 1.15).toLocaleString()} ريال</span>
              </div>
            </div>
          </div>
        </div>
