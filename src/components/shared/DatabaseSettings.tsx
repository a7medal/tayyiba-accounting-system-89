...
export function DatabaseSettings() {
  const [dbType, setDbType] = useState<string>(localStorage.getItem('appDbType') || 'sqlite');
  const [isConnected, setIsConnected] = useState<boolean>(false);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const { toast } = useToast();

  useEffect(() => {
    setIsConnected(AppDatabase.isConnected());
  }, []);

  const handleDbTypeChange = (value: string) => {
    setDbType(value);
    AppDatabase.setDbType(value);
    ...
    if (dbType === 'sqlite') {
      try {
        ...
        if (data) {
          ...
          a.href = url;
          a.download = `database_${new Date().toISOString().split('T')[0]}.db`;
  ...
  return (
    <Card className="shadow-md">
      <CardHeader>
        ...
        <CardDescription>
          إدارة اتصال وإعدادات قاعدة البيانات للتطبيق
        </CardDescription>
      </CardHeader>
